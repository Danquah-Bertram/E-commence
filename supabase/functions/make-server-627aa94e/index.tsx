...
app.post("/auth/login", async (c) => {
  try {
    ...
    if (user.suspended) return c.json({ error: "Your account has been suspended. Contact danquahbertram26@gmail.com or call 0596215537." }, 403);
    await logActivity("login", `${user.role === "admin" ? "Admin" : "Customer"} logged in: ${email}`, user.name);
    const { passwordHash: _, ...safe } = user;
